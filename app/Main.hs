  ...
  (\s -> s ++ "\n") $
    intercalate "\n" $
      "Command names (the first token of the command) are not case sensitive.\n"
        ...
        : ":se\n"
        : "[The commands for showing and typing the environment can also be used for environment pages, as follows]\n"
        : ":se <page_number>\n"
        : ":te <page_number>\n"
        : "[Page numbers start at 1]\n"
        : "[Programs may be executed directly in the command line; LTIBDT will show and then type it]\n"
        : "<program>"
        : []
